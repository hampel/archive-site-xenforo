...
class ArchiveUser extends Command
{
	...
	{
		$this
			->setName('archive:archive-user')
			->setDescription('Archive a single user')
			->addArgument(
				'user',
                InputArgument::REQUIRED,
				"User_id, username or email address of user to archive"
			);
	}
	...
	protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln("<question>" . \XF::phrase('hampel_archivesite_are_you_sure_archiving_x', ['user' => $user->username]) . "</question>");
        ...
        if (!$archiveRepo->archiveUser($user)) {
            $output->writeln("<error>" . \XF::phrase('hampel_archivesite_error_archiving_user_x', ['user' => $userArg]) . "</error>");
            return 1;
        }
